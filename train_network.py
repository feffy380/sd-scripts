...
class NetworkTrainer:
    ...
    def train(self, args):
        ...
        # move text encoder to target device, cast if not being trained
        for t_enc in text_encoders:
            t_enc.to(accelerator.device, dtype=(weight_dtype if not args.continue_inversion else None))
        ...
        for t_enc in text_encoders:
            ...
            if t_enc.device.type != "cpu" and not args.continue_inversion:
        ...
        if args.continue_inversion:
            ...
            for tokenizer, token_ids, t_enc in zip(tokenizers, token_ids_list, text_encoders):
                ...
                unwrapped_te = accelerator.unwrap_model(t_enc)
                unwrapped_te.text_model.encoder.requires_grad_(False)
                unwrapped_te.text_model.final_layer_norm.requires_grad_(False)
                unwrapped_te.text_model.embeddings.position_embedding.requires_grad_(False)
                # unwrapped_te.text_model.embeddings.token_embedding.requires_grad_(True)
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(training_model):
                    ...
                    if "latents" in batch and batch["latents"] is not None:
                        ...
                        with torch.no_grad():
                            # latentに変換
                            latents = vae.encode(batch["images"].to(dtype=vae_dtype)).latent_dist.mean.to(dtype=weight_dtype)
                    ...
                    if args.learned_timestep_weighting:
                        loss = apply_learned_weight(loss, timesteps, noise_scheduler)
