...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def cache_latents(self, vae, vae_batch_size=1, cache_to_disk=False, is_main_process=True):
        ...
        class ImageDataset(torch.utils.data.Dataset):
            def __init__(self, batches, alpha, random_crop):
            ...
            def __getitem__(self, idx):
                ...
                for info in image_infos:
                    image = load_image(info.absolute_path, self.alpha) if info.image is None else info.image
                ...
                return image_infos, img_tensors, alpha_masks
        ...
        image_dataset = ImageDataset(batches, subset.alpha_mask, subset.random_crop)
        ...
        for batch in tqdm(image_dataloader, smoothing=0.01, total=len(image_dataloader)):
            infos, img_tensors, alpha_masks = batch[0]
            cache_batch_latents(vae, cache_to_disk, infos, img_tensors, alpha_masks, flip_aug)
    ...
    def __getitem__(self, index):
        ...
        for i, image_key in enumerate(bucket[image_index : image_index + bucket_batch_size]):
            ...
            if self.dreambooth_dataset_delegate.enable_bucket:
                ...
                cond_img = cond_img.resize(image_info.resized_size, Image.Resampling.LANCZOS)
                # cond_img = cv2.resize(
                ...
                if cond_img.shape[0] != target_size_hw[0] or cond_img.shape[1] != target_size_hw[1]:
                    cond_img = cond_img.resize((int(target_size_hw[1]), int(target_size_hw[0])), Image.Resampling.LANCZOS)
                    # cond_img = cv2.resize(
...
def cache_batch_latents(
    vae: AutoencoderKL, cache_to_disk: bool, image_infos: List[ImageInfo], img_tensors: torch.Tensor, alpha_masks: List[torch.Tensor], flip_aug: bool
) -> None:
    r"""
