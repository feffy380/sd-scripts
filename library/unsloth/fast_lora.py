...
# with modifications by feffy380
#
...
class LoRA_QKV(torch.autograd.Function):
    ...
    def forward(ctx, X : torch.Tensor, context : torch.Tensor,
        ...
        ctx.save_for_backward(X, context, QA, QB, KA, KB, VA, VB,)
        ctx.inplace = inplace
        return Q, K, V
    pass
    ...
    def backward(ctx, dQ, dK, dV):
        QW, QW_quant, QS, KW, KW_quant, KS, VW, VW_quant, VS = \
            ctx.custom_saved_tensors
        X, context, QA, QB, KA, KB, VA, VB, = ctx.saved_tensors

        batch, seq_len, hd = X.shape
        cbatch, cseq_len, chd = context.shape
        ...
        # d_KA *= KS
        # d_KB *= KS
        d_KA.addmm_(context.t(), dK @ KB.t(), alpha = KS, beta = 0)
        d_KB.addmm_(KA.t() @ context.t(), dK, alpha = KS, beta = 0)
        ...
        # d_VA *= VS
        # d_VB *= VS
        d_VA.addmm_(context.t(), dV @ VB.t(), alpha = VS, beta = 0)
        d_VB.addmm_(VA.t() @ context.t(), dV, alpha = VS, beta = 0)
        ...
        # dX += dK @ KW.t()
        d_context = torch.matmul(dK, KW.t())
        del KW
        # dX += dK @ KB.to(dtype).t() @ (KS * KA.to(dtype).t())
        d_context.addmm_(dK @ KB.t(), KA.t(), alpha = KS)

        # dV
        VW = fast_dequantize(VW.t(), VW_quant)
        # dX += dV @ VW.t()
        d_context.addmm_(dV, VW.t())
        del VW
