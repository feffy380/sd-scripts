...
import torch
import re
from library.unsloth import fast_lora
...
class LoRAModule(torch.nn.Module):
    ...
    def apply_to(self):
        # for unsloth kernel
        self.org_module.loraA = [self.lora_down]
        self.org_module.loraB = [self.lora_up]
        self.org_module.loraS = self.scale
...
class LoRANetwork(torch.nn.Module):
    ...
    def apply_to(self, text_encoder, unet: torch.nn.Module, apply_text_encoder=True, apply_unet=True):
        ...
        for name, module in unet.named_modules():
            if module.__class__.__name__ == "FeedForward":
                module.forward = types.MethodType(fast_lora.apply_lora_mlp_geglu_exact, module)
            elif module.__class__.__name__ == "CrossAttention":
                module.apply_qkv = types.MethodType(fast_lora.apply_lora_qkv, module)
                module.to_out[0].forward = types.MethodType(fast_lora.apply_lora_o, module.to_out[0])
