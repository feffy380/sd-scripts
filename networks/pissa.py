...
class LoRAModule(torch.nn.Module):
    ...
    def __init__(
        ...
        # # same as microsoft's
        # torch.nn.init.kaiming_uniform_(self.lora_down.weight, a=math.sqrt(5)) # A
        # torch.nn.init.zeros_(self.lora_up.weight) # B

        # initialization. note: lora_down = A, lora_up = B
        # source: https://github.com/huggingface/peft/pull/1626
        weight = org_module.weight
        dtype = weight.dtype
        device = weight.device
        if len(weight.data.shape) == 2:
            res = weight.data
        elif len(weight.data.shape) == 4 and weight.data.shape[2:4] == (1, 1):
            # conv2d 1x1
            res = weight.data.reshape(weight.data.shape[:2])
        else:
            raise NotImplementedError("PiSSA not implemented for Conv2d 3x3")

        # PiSSA init
        res: torch.Tensor = res.float()
        Vr, Sr, Ur = torch.svd_lowrank(res, self.lora_dim, niter=16)
        Sr /= self.scale
        Uhr = Ur.t()
        lora_down = torch.diag(torch.sqrt(Sr)) @ Uhr
        lora_up = Vr @ torch.diag(torch.sqrt(Sr))
        res = res - self.scale * lora_up @ lora_down

        if len(weight.data.shape) == 4:
            # restore conv2d 1x1
            lora_down = lora_down.unsqueeze(2).unsqueeze(3)
            lora_up = lora_up.unsqueeze(2).unsqueeze(3)
            res = res.unsqueeze(2).unsqueeze(3)
        lora_dtype = self.lora_down.weight.dtype
        self.lora_down.weight.data = lora_down.to(lora_dtype)
        self.lora_up.weight.data = lora_up.to(lora_dtype)
        org_module.weight.data = res.to(dtype)

        # copy initial weights to init weights for conversion to LoRA later
        self._init_down = self.lora_down.weight.data.detach().clone()
        self._init_up = self.lora_up.weight.data.detach().clone()
    ...
    def convert_to_lora(self):
        ...
        # W + dW = W^res + A \times B
        # dW = A \times B - A_0 \times B_0 = [A + A_0] \times [B - B_0]^T
        device = self.lora_down.weight.device
        delta_w_A = torch.cat([self.lora_down.weight.data, self._init_down.to(device)], dim=0)
        delta_w_B = torch.cat([self.lora_up.weight.data, -self._init_up.to(device)], dim=1)
        # backup weights
        self.lora_down_backup = self.lora_down.weight.data
        self.lora_up_backup = self.lora_up.weight.data
        # replace with delta W
        self.lora_down.weight.data = delta_w_A
        self.lora_up.weight.data = delta_w_B
        self.alpha *= 2

    def convert_to_pissa(self):
        self.alpha /= 2
        self.lora_down.weight.data = self.lora_down_backup
        self.lora_up.weight.data = self.lora_up_backup
        del self.lora_down_backup, self.lora_up_backup
...
class LoRANetwork(torch.nn.Module):
    ...
    def prepare_optimizer_params(self, text_encoder_lr, unet_lr, default_lr):
        ...
        def enumerate_params(loras: List[LoRAModule]) -> List:
            params = []
            for lora in loras:
    ...
    def save_weights(self, file, dtype, metadata):
        ...
        for lora in loras:
            lora.convert_to_lora()

        state_dict = self.state_dict()
        keys = set()
        for key in state_dict.keys():
            keys.add(key.split(".", maxsplit=1)[-1])
        assert len(keys) == 3, f"state_dict contains extra keys: {keys}"
        ...
        # convert back to PiSSA
        for lora in loras:
            lora.convert_to_pissa()
